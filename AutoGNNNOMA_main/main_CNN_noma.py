...
from algs.ConvNet import ConvNet
import torch
from lib.datasets.SearchDataWirelessComm import DatasetNOMA


def train_cnn(xloader, network, args, scheduler, w_optimizer, epoch_str, print_freq, logger):
    ...
    for step, (inputs,_) in enumerate(xloader):
        inputs_CNN = build_CNN_inputs(inputs, args)
        scheduler.update(None, 1.0 * step / len(xloader))
        data_time.update(time.time() - end)  # measure data loading time
        w_optimizer.zero_grad()
        NN_output = network(inputs_CNN)
        # calculate loss
        loss, rate, SIC = sr_loss(inputs_CNN, NN_output, None, None, args)
        loss.backward()
...
def test_CNN(xloader, network, args):
    data_time, batch_time = AverageMeter(), AverageMeter()
    ...
    with torch.no_grad():
        for step, (valid_inputs) in enumerate(xloader):
            ...
            # prediction
            valid_inputs_CNN = build_CNN_inputs(valid_inputs, args)
            NN_output = network(valid_inputs_CNN)
            arch_loss, arch_rate, arch_SIC = sr_loss(valid_inputs_CNN, NN_output, None, None, args)
...
def main_train(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        search_w_loss, search_mean_rate, search_mean_SIC, search_comm_cost = train_cnn(train_loader, network, xargs, w_scheduler, w_optimizer, epoch_str, xargs.print_freq, logger)
        ...
        valid_a_loss, valid_rate, valid_SIC, valid_comm_cost = test_CNN(valid_loader, network, xargs)
...
if __name__=='__main__':
    ...
    parser.add_argument('--num_layer', type=int, default=16, help='The number of neural layers.')
    # leraning
    ...
    parser.add_argument('--save_dir', type=str, default='./result/ConvNet/', help='Folder to save checkpoints and log.')
